...
function insert() {
  ...
  if (count == -1) {
    ...
    node.innerHTML = `<div class="arrow_block"><div class="arrow-f"></div><div class="arrow-d"></div></div><div class='pre'>${
      stack[count - 1]
    }</div><div class='data'>${data}</div><div class='next'>null</div>`;
    const head = document.createElement("div");
    head.classList.add("pointer");
    ...
    count = count + 1;
    box.lastElementChild.innerHTML = `<div class="arrow_block"><div class="arrow-f"></div><div class="arrow-d"></div></div><div class='pre'>${
      stack[count - 2]
    }</div><div class='data'>${dt}</div><div class='next'>${
      stack[count]
    }</div>`;
    const data = input.value;
    i = i + 1;
...
function del() {
  ...
  if (count == 0) {
    ...
    box.lastElementChild.innerHTML = `<div class="del-ani"><div class="arrow_block"><div class="arrow-f"></div><div class="arrow-d"></div></div><div class='pre'>${
      stack[count - 1]
    }</div><div class='data'>${
      stack1[i]
    }</div><div class='next'>null</div></div>`;
    setTimeout(() => {
      box.removeChild(box.lastElementChild);
      pointer.removeChild(pointer.lastElementChild);
	  if(box.children.length > 1){
		box.lastElementChild.innerHTML = `<div class="arrow_block"><div class="arrow-f"></div><div class="arrow-d"></div></div><div class='pre'>${
			...
			stack1[i]
		  }</div><div class='next'>null</div>`;
	  }else{
		box.lastElementChild.innerHTML = `<div class='pre'>null</div><div class='data'>${
			stack1[i]
		  }</div><div class='next'>null</div>`;
	  }
    }, 1200);
...
function stop3() {
  ...
  setTimeout(() => {
    box.lastElementChild.innerHTML = `<div class="arrow_block"><div class="arrow-f"></div><div class="arrow-d"></div></div><div class='pre'>${
      stack[count - 1]
    }</div><div class='data'>${stack1[i]}</div><div class='next'>null</div>`;
  }, 1200);
}
